output "ssh_connection_string_for_ec2" {
  value = format("%s%s", "ssh -i /home/vinayak/terraform/terraform-jenkins/jenkins_key ubuntu@", aws_instance.jenkins_ec2_instance_ip.public_ip)
}
...
output "jenkins_eip_public_ip" {
  ...
  description = "Elastic IP associated with the Jenkins EC2 instance"
}